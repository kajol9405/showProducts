package com.example.kajol.showproducts.Adapter
...
class CategoryRecyclerAdapter(val context: Context, val categories: List<Category>, val itemClick: (Category) -> Unit): RecyclerView.Adapter<CategoryRecyclerAdapter.Holder>() {
    ...
    inner class Holder(itemView: View?, val itemClick: (Category) -> Unit) : RecyclerView.ViewHolder(itemView) {
        val categoryImage = itemView?.findViewById<ImageView>(R.id.selectedCategoryImage)
        val categoryName = itemView?.findViewById<TextView>(R.id.categoryItemName)
